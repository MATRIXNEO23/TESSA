...
SQLITE_FILE = INDEX_DIR / "tessa_memory.sqlite3"
MANIFEST_FILE = RAG_ROOT / "memory_manifest.json"
VISUAL_INDEX_FILE = INDEX_DIR / "TESSA_VISUAL_CHRONOLOGY.md"
CURRENT_CONTEXT_FILE = INDEX_DIR / "CURRENT_CONTEXT.md"
FAST_RECALL_FILE = INDEX_DIR / "TESSA_FAST_RECALL.md"
...
def expand_sources(manifest: dict) -> list[tuple[Path, dict]]:
    ...
    # Owner-scoped RAG sources are explicit in the manifest. This keeps Tessa
    # memories/transcripts searchable without ever ingesting Tessa's memory.
    rag_excludes = manifest.get(
        "rag_exclude",
        ["rag/index/**", "rag/memories/gptina/**"],
    )
    ...
    # Backward-compatible fallback for older manifests: include Tessa/legacy
        ...
        if memories.exists():
            for pattern in ("*.md", "*.json"):
                for p in memories.rglob(pattern):
                    rp = rel(p)
                    if excluded(rp, ["rag/memories/gptina/**", "rag/memories/README.md"]):
                        continue
                    found[rp] = (
                        p,
...
def query_profile(query: str) -> tuple[set[str], str | None]:
    lower = query.casefold()
    profile: set[str] = set()
    if any(w in lower for w in ("immagine", "foto", "fotina", "volto", "visual", "images/", "media/")):
        profile.add("visual")
...
def sqlite_search(
    ...
    for row in rows:
        ...
        if "temporal" in profile and kind in {
            ...
            "tessa_transcript", "raw_session", "chronicle"
        }:
            score *= 1.22
        if "current" in profile:
            if kind in {"live_context", "micro_checkpoint"}:
                score *= 1.35
            elif kind in {"current_router", "tessa_memory", "checkpoint"}:
                score *= 1.20
            if kind in {
...
def bm25_search(
    ...
    for d, toks in zip(docs, tokenized):
        ...
        # "tu + tessa = casa") beat generic documents containing the words
        ...
        if "visual" in profile and kind in {"visual_router", "visual_context"}:
            score *= 1.35
        if "temporal" in profile and kind in {
            "chronology_router", "checkpoint", "tessa_transcript", "raw_session", "chronicle"
        }:
            score *= 1.22
        if "current" in profile:
            if kind in {"current_router", "tessa_memory", "checkpoint"}:
                score *= 1.20
...
def create_image_link(
    ...
    media_root = (ROOT / "images").resolve()
    ...
    record = {
        "schema_version": 1,
        "owner": "tessa",
        "image_id": "tessa-image-" + hashlib.sha256(
...
def verify_future_memory_schema(manifest: dict) -> None:
    ...
    memories = RAG_ROOT / "memories" / "tessa"
    ...
    for path in sorted(memories.rglob("*.md")):
        ...
        if "owner: tessa" not in raw[:2200]:
            errors.append(f"{rp}: owner must be tessa")
    if errors:
...
def verify_boundary() -> None:
    ...
    forbidden = [rel(p) for p, _ in sources if rel(p).startswith("rag/memories/gptina/")]
    if forbidden:
        fail(f"GPTina-owned memories entered Tessa index: {forbidden}")
        ...
        if link_dir.is_dir():
            for record_path in link_dir.rglob("*.json"):
                ...
                if record.get("owner") != "tessa":
                    fail(f"Image-link owner must be tessa: {rel(record_path)}")

                image_file = ROOT / image_path
                if not image_file.is_file():
                    fail(
                        f"Image-link points to missing image file: "
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Tessa isolated long-term RAG memory")
    ...
    ep = sub.add_parser("find-exact", help="Find an exact phrase in current Tessa sources")
