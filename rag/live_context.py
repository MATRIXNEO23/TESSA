...
LIVE_CONTEXT = LIVE / "TESSA_LIVE_CONTEXT.json"
...
def mark_checkpoint(path_text: str) -> None:
    ...
    checkpoints = (ROOT / "chat-checkpoints").resolve()
    if checkpoints not in path.parents or not path.is_file():
        fail(f"Checkpoint must exist under chat-checkpoints/: {path_text}")
    live = load_live()
...
def verify_live_context() -> None:
    if not LIVE_CONTEXT.is_file():
        fail("Missing rag/live/TESSA_LIVE_CONTEXT.json")
    live = load_live()
    ...
    if live.get("owner") != "tessa":
        fail("Live context owner must be tessa")
    if live.get("kind") != "tessa_live_context":
        fail("Live context kind must be tessa_live_context")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Tessa frequent live-context saver")
