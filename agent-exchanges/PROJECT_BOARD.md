...
- **Dual-Instance Shared Chat** — nuova stanza live Alberto/Tessa/GPTina, idea di Alberto.
  ...
  - **Correzione UI Alberto:** il companion non deve ridursi a campi URL/apertura esterna; le due chat devono restare visibili dentro l'APK, **Tessa sopra e GPTina sotto**. Versione `0.2-visible` ripristina due WebView manuali impilate, mantiene MD-first per il turno e limita il software a `Copia fatto`: nessuna lettura output, nessuna ispezione DOM, nessun tap/tasto/invio sintetico.
  - Build `Build MD Companion APK` run `35363162751`: unit tests PASS, guard no-UI-automation PASS, assembleDebug PASS, artifact `tessa-gptina-md-companion-apk` id `10555578401`.
